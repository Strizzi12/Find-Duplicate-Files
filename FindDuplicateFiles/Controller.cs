...
namespace FindDuplicateFiles
{
  public class Controller
  {
    ...
    private List<string> GetFiles(string path, List<string> filter, int? depth = null)
    {
      ...
      SafeFileEnumerator safeEnumeratorHandler = new SafeFileEnumerator(path, "*", SearchOption.AllDirectories, depth); //ignore filter here with "*"

      var safeEnumerator = safeEnumeratorHandler.GetEnumerator();

      while (safeEnumerator.MoveNext())
      {
        if (safeEnumerator.Current?.Extension != "")
        {
          filesInPath.Add(safeEnumerator.Current.FullName);
          //Console.WriteLine(safeEnumerator.Current);
        }
      }

      safeEnumerator.Dispose();
      return FilterFiles(filesInPath, filter);
    }
